...
local BINDINGS = {
  ...
  -- ─── Displays (Cmd+Ctrl; Paneru natives are single-hop "next display"
  -- commands — with two monitors "next" wraps to the other one) ───
  ["window nextdisplay"] = "cmd + ctrl - rightarrow",              -- move + follow
  ["window nextdisplaysend"] = "cmd + ctrl + shift - rightarrow",  -- move, stay
  ["mouse nextdisplay"] = "cmd + ctrl - uparrow",                  -- warp cursor; focus_follows_mouse moves focus along
...
-- ─── Display navigation ───
-- Native Paneru commands only (no Swift/CG helpers): cross-display focus
-- without moving a window is reached via the mouse warp (horizontal_mouse_warp
-- above) + focus_follows_mouse — slide the cursor to a screen edge and focus
-- follows it across. With two monitors arranged vertically in macOS, "next
-- display" wraps between them.
...
paneru.bind("cmd + shift - ,", function()
