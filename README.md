...
| `install-helpers` | Install the shortcut cheat-sheet helpers (`generate-shortcuts-json`, `display-shortcuts`) into `~/.config/mac-scrolling-wm/helpers/` and install the macOS cheat-sheet viewer app (fetches a pre-built release from GitHub at `iv-lite/mac-cheatsheet-viewer`, falls back to a local source build) |
...
| Shortcut | Action |
|---|---|
| `Cmd` + `Ctrl` + `→` | Move focused window to next display (follow; "next" wraps with two monitors) |
| `Cmd` + `Ctrl` + `Shift` + `→` | Move focused window to next display (stay on current) |
| `Cmd` + `Ctrl` + `↑` | Warp mouse to next display (`focus_follows_mouse` shifts focus along) |
...
**How cross-display focus works without a dedicated shortcut:** Paneru does not
have a first-class "focus other display" command — cross-display focus is reached
instead through the **mouse warp** (`horizontal_mouse_warp = -1` in the config)
combined with `focus_follows_mouse`: slide the cursor to a screen edge and focus
follows it across. This is the primary way to shift focus between displays with
the keyboard, and it feels left/right exactly as if the monitors were physically
side-by-side.

With exactly two displays, the native `window nextdisplay` (and the mouse warp)
wraps — pressing `Cmd+Ctrl+→` once moves the window to the other monitor, and
pressing it again brings it back. No "previous display" command exists in
Paneru's native API, so there is no directional move or focus shortcut; the
single `→` wrap is the only path, and it works correctly for both focus and
window movement with two monitors.
