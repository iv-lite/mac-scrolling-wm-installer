...
cmd_check() {
  ...
  guest "command -v tccutil-rs && tccutil-rs --version || echo '(tccutil-rs missing)'"
  echo ""
  ...
  guest "grep -q 'default_ratio *= *1\\.0' ~/.config/paneru/init.lua && grep -q 'center_single_column *= *true' ~/.config/paneru/init.lua && grep -q 'maximize_tiled_windows *= *true' ~/.config/paneru/init.lua && grep -q 'reap_empty_workspaces *= *true' ~/.config/paneru/init.lua && grep -q 'create_virtual_workspace_automatically *= *true' ~/.config/paneru/init.lua && grep -q 'animations *= *true' ~/.config/paneru/init.lua && grep -q 'animation_duration_ms *= *250' ~/.config/paneru/init.lua && grep -q 'virtual_workspace_animations *= *false' ~/.config/paneru/init.lua && grep -q 'ax_writer *= *true' ~/.config/paneru/init.lua && grep -q 'insert_windows_mid_strip *= *true' ~/.config/paneru/init.lua && grep -q 'missing_windows *= *\"drop\"' ~/.config/paneru/init.lua && grep -q 'padding *= *{ *top *= *8' ~/.config/paneru/init.lua && grep -q 'gaps *= *{ *horizontal *= *8' ~/.config/paneru/init.lua && grep -q 'vertical *= *8' ~/.config/paneru/init.lua && grep -q 'width *= *2\\.0' ~/.config/paneru/init.lua && grep -q 'border *= *{ *enabled *= *false' ~/.config/paneru/init.lua && ! grep -q 'animation_speed *=' ~/.config/paneru/init.lua && ! grep -q 'drag_friction_[a-z_]* *=' ~/.config/paneru/init.lua && ! grep -q 'experimental_vsync *=' ~/.config/paneru/init.lua && ! grep -q 'left_drag_scrolls_strip *=' ~/.config/paneru/init.lua && ! grep -q 'ffm_drag_suppress_[a-z_]* *=' ~/.config/paneru/init.lua && echo '(default_ratio + center_single_column + maximize + reap/create + animations true + duration 250 + virtual_workspace_animations false + ax_writer + insert_windows_mid_strip + missing_windows drop + padding 8 + gaps 8 + slim borders, stale keys absent)' || echo '(MISSING new options)'"
  echo ""
